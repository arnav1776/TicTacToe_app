...
class _HomePageState extends State<HomePage> {
  ...
  void playGame(GameButton gb) {
    setState(() {
      if (activePlayer == 1) {
        gb.text = 'X';
        gb.bg = Colors.orange.shade300;
        activePlayer = 2;
        player1.add(gb.id);
      } else {
        gb.text = 'O';
        gb.bg = Colors.lime[600];
        activePlayer = 1;
        player2.add(gb.id);
      }
      gb.enabled = false;
      ...
      if (winner == -1 && buttonsList.every((p) => p.text != '')) {
        ...
        (activePlayer == 2 && _option == _options.elementAt(1))
            ? autoPlayZero()
            : null;
      }
    });
  ...
  int checkWinner() {
    ...
    if (winner == 1 || winner == 2) {
      showDialog(
          context: context,
          builder: (_) => new CustomDialog(
              'Player $winner Won', 'Press Reset to Play Again', resetGame));
    }
    return winner;
  }
              ...
              items: _options.map((String value) {
                return new DropdownMenuItem(
                  ...
                  child: new Text(value,
                      style: new TextStyle(
                          color: Colors.black45, fontWeight: FontWeight.bold, fontSize: 20)),
                );
              }).toList(),
                ...
                itemBuilder: (context, i) => new SizedBox(
                    ...
                    child: new RaisedButton(
                      ...
                      child: new Text(buttonsList[i].text,
                          style: new TextStyle(
                              color: Colors.black87, fontSize: 30.00)),
